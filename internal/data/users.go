...
// Define a custom ErrDuplicatedEmail error.
var (
...
// the Set() commmand calculates the bcrypt hash of a plaintext password, and stores both
...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8, "password", "must be at least 8 bytes in length")
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	// useful sanity check to include here, but its not a problem with the data
	// provided by the client.  So rather that adding an error to the validation map we
...
// version fields are all automatically generated by our database, se we use the
...
// Becasue we have a UNIQUE constraint on the email column, the SQL query will only
